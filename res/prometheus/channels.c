...
static void channels_scrape_cb(struct ast_str **response)
{
	...
	for (i = 0; i < ARRAY_LEN(channel_metric_defs); i++) {
		it_chans = ao2_iterator_init(channels, 0);
		for (j = 0; (snapshot = ao2_iterator_next(&it_chans)); ao2_ref(snapshot, -1), j++) {
			int index = i * num_channels + j;

			channel_metrics[index].type = PROMETHEUS_METRIC_GAUGE;
			ast_copy_string(channel_metrics[index].name, channel_metric_defs[i].name, sizeof(channel_metrics[index].name));
			channel_metrics[index].help = channel_metric_defs[i].help;
			...
			channel_metric_defs[i].get_value(&channel_metrics[index], snapshot);

			if (j > 0) {
				AST_LIST_INSERT_TAIL(&channel_metrics[i * num_channels].children, &channel_metrics[index], entry);
			}
		}
		ao2_iterator_destroy(&it_chans);

		prometheus_metric_to_string(&channel_metrics[i * num_channels], response);
	}
