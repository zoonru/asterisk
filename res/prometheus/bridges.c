...
static void bridges_scrape_cb(struct ast_str **response)
{
	...
	for (i = 0; i < ARRAY_LEN(bridge_metric_defs); i++) {
		it_bridges = ao2_iterator_init(bridges, 0);
		for (j = 0; (bridge = ao2_iterator_next(&it_bridges)); ao2_ref(bridge, -1), j++) {
			struct ast_bridge_snapshot *snapshot = ast_bridge_get_snapshot(bridge);
			int index = i * num_bridges + j;

			bridge_metrics[index].type = PROMETHEUS_METRIC_GAUGE;
			ast_copy_string(bridge_metrics[index].name, bridge_metric_defs[i].name, sizeof(bridge_metrics[index].name));
			bridge_metrics[index].help = bridge_metric_defs[i].help;
			...
			bridge_metric_defs[i].get_value(&bridge_metrics[index], snapshot);

			if (j != 0) {
				AST_LIST_INSERT_TAIL(&bridge_metrics[i * num_bridges].children, &bridge_metrics[index], entry);
			}
		...
		prometheus_metric_to_string(&bridge_metrics[i * num_bridges], response);
	}
